...
```bash
bash update_give.sh [-r <give_root>] [-t <toolbox_dir>] [-e <example_dir>]
```
...
- `-t <toolbox_dir>`: (required) The directory of the bash scripts of GIVE-Toolbox. The default value is `/usr/local/bin`, the same as the settings in GIVE-Docker.
