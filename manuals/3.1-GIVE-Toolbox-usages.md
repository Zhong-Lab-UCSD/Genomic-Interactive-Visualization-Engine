...
```bash
bash config_host.sh [-r <give_root>] [-u <mysqlu>] [-p <mysqlp>] [-d <host_domain[:port]>]
```
...
- `-d <host_domain[:port]>`: (optional) If the local machine has web service and you want to access GIVE through web service, you need to set `<host_domain[:port]>` in the file "`<give_root>/html/components/basic-func/constants.js`". The `<host_domain>` value we set in the GIVE Data Hub is "`https://www.givengine.org`". Specify the `[:port]` part if you are using a port other than `80`, for example: "`http://www.givengine.org:40080`".
- `-h`: show usage help
