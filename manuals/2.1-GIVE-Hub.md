...
You may filter all the tracks supported for the current reference via the FILTER button. After you click the button, the filter window will appear. You may choose the field and value for the criterion that tracks must match. Currently, the data type (ChIP-Seq, RNA-Seq, *etc.*), the cell line or tissue name, and the lab name of the tracks are supported.

![GIVE Data Hub filter](images/2-1-Give-Hub-filter.png)

Once a filter is active, the FILTER button will show the criterion of the current filter and you may use the <img src="images/2-1-icon-clear.svg" height="16" width="16"> button to clear it, or click the FILTER button again to change the criterion.

Applying a filter __will not affect__ any of your previous track selections. Therefore, you may use the filter function multiple times to select a combination of tracks matching different criteria.
