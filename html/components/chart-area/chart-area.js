  ...
  class ChartArea extends mixinBehaviors([
    ...
    static get properties () {
      return {
        ...
        /**
         * The highlighted coordinates, an array of array(s) (one for each
         * reference) of highlighted coordinates, in JSON format.
         * @type {string}
         */
        highlight: {
          type: String,
    ...
    ready () {
      ...
      this._updatingHighlights = false
      super.ready()
    ...
    _syncCoordinatesJsonToArray (newCoorindateValue) {
      ...
      let refObjArray = this._refObjArray
      this._coordinateArray = give.getValueArray(newCoorindateValue,
        this.numOfSubs,
        (!this.allowEmptyCoordinates && refObjArray)
          ? refObjArray.map((refObj, index) => {
    ...
    updateDom (newCoordinates) {
      ...
      this._readiness = false
      if (this._updatingHighlights) {
        this._initHighlightValue()
      }
      return Promise.all(
    ...
    get _refObjArray () {
      try {
        return this._refArray.map(ref => give.RefObject.findRefByDb(ref)
        )
    ...
    _highlightChanged (newValue, oldValue) {
      if (newValue) {
        this._highlightRegionStrings = JSON.parse(newValue)
        this._updatingHighlights = true
        let refObjArray = this._refObjArray
        if (refObjArray && refObjArray.length) {
          this.getRefsReadyPromise().then(
            () => this._initHighlightValue()
    ...
    _initHighlightValue () {
      let refObjArray = this._refObjArray
      if (refObjArray) {
        this._highlightArray = this._highlightRegionStrings.map(
          (highlightRegionInRef, index) => {
            if (Array.isArray(highlightRegionInRef)) {
              return highlightRegionInRef.map(
                (regionString, regionIndex) =>
                  new give.ChromRegion(regionString, refObjArray[index], {
                    colorIndex: regionIndex
                  })
              )
            }
            return [
              new give.ChromRegion(highlightRegionInRef, refObjArray[index], {
                colorIndex: regionIndex
              })
            ]
          }
        )
        refObjArray.forEach((refObj, index) => refObj.initializeHighlights(
          this._highlightArray[index % this._highlightArray.length])
        )
        this._updatingHighlights = false
      }
    }
    ...
    _urlParamToProp (params) {
      // rewrite methods to single out `ref`, `coordinates` and `highlight`
