...
var GIVe = (function (give) {
  ...
  class ChartWindow extends
    ...
    <svg id="highlightHolder" version="1.1" baseProfile="full">
    </svg>
    <template is="dom-if" if="[[showTutorial]]">
      <tutorial-highlight id="coorTrackTutorial" tutorial-key="coor-track-tutorial" dom-target-id="[[_coorDomSvgId]]">
        <div>
          You may drag any track to move, or use the mouse wheel within the
          coordinate track to zoom in/out. <br>
          If you don't have a mouse wheel but have a trackpad, you may zoom
          in/out by:
          <ul>
...
`;
    ...
    updateSvgHeightLocation (width) {
      ...
      this.updateHighlightSvgHeightLocation()
    }
    ...
    updateTracks (newVWindow, width) {
      ...
      this.drawingPromise = this._trackPromiseAggregator.aggregate(
        this._trackDoms.concat(this._hiddenTrackDoms)
          .concat(this._highlightTrackDom),
    ...
    updateHighlightDom () {
      if (!this._highlightTrackDom) {
        this._highlightTrackDom = this.refObj.highlightTrack.createDomObj(
          this._createTrackProp({
            'height': this.$.chartHolder.clientHeight,
      ...
      while (this.$.highlightHolder.firstChild) {
        this.$.highlightHolder.removeChild(this.$.highlightHolder.firstChild)
      }
      this.$.highlightHolder.appendChild(this._highlightTrackDom.trackSvg)
    }

    updateHighlightSvgHeightLocation () {
      this._highlightTrackDom.updateSize(
        this.windowWidth, this.$.chartHolder.clientHeight)
      this._highlightTrackDom.update(null, null, null, true)
        .catch(ignore => {})
    }
  }
