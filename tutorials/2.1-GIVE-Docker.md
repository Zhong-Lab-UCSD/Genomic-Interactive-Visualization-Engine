...
### Update to the latest GIVE-Docker / GIVE
...
- Update GIVE components in existing container
  ...
  We provide a bash script `update_give.sh` in GIVE-Toolbox. Here is an example of using it to update the GIVE components in an existing GIVE-Docker container `give`.
    ...
    docker exec -t -i give /bin/bash
    bash update_give.sh -r /var/www/give -t /usr/local/bin -e /tmp/example_data
    ```
  For more usage information of `update_give.sh`, you can check the [Manual 3.1: GIVE-Toolbox Usage](../manuals/3.1-GIVE-Toolbox-usage.md#10-update_givesh). 
