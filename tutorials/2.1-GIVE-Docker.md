...
The traditional way to deploy web service needs to install dependencies and configure environment at the operating system level. Although we already supplied a comprehensive manual for system-level [installation of GIVE](../manuals/), it's hard for users who lack experience in Linux administration to follow. Even for expert users, it's always annoying to do it. 
...
### Configure The Host Server for Web Accessibility
  After the 4 steps of deployment, you have got a local GIVE service. You can use it like a stondalone application. If your host server is web accessible, then you can configure the GIVE Docker container for using GIVE service remotely, like our GIVE Data Hub. 
