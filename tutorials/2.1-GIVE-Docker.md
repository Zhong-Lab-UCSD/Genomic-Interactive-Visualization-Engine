...
The traditional way to deploy web service needs to install dependencies and configure environment at the operating system level. Although we already supplied a comprehensive manual for system-level [installation of GIVE](../manuals/), #it would be better if this link could be the link of customer installation directly# it's hard for users who lack experience in Linux administration to follow. Even for expert users, it's always annoying to do it. 
...
- **Test the built-in [Demo2-ENCODE2_ChIA-PET](../gallery/Demo2-ENCODE2_ChIA-PET) genome browser**
  ...
  In the previous `docker run` command, `-p` options map three ports for communication between the operating system and GIVE container. With the mapping ports `40080 -> 80`(for http) and `40443 -> 443`(for https), #add explaination for the third mapping port# we can use web browser to open the built-in Demo2-ENCODE2_ChIA-PET genome browser with following URLs without need for internet connection
...
### Configure The Host Server for Web Accessibility
  ...
  We provide a shell script tool `config_host.sh` for configuring the host server. The following command is an example that implementing host server by using `config_host.sh` in the GIVE-Toolbox. For more information, please read [GIVE-Toolbox tutorial](3-GIVE-Toolbox.md)
  ```
...
- Login to GIVE container
 ...
  You can login to the running GIVE container `give` as root for further operations.
  ...
  ```
  Alternatively, you can also directly login to the container `give` in MySQL database.
  ```
  docker exec -t -i give mysql -uroot -p
  ```
  The password of MySQL `root` account is `Admin2015`. #why not type the password directly in the command?#
- Transfer files
...
- Stop, restart and remove running container
  ...
  You can use `docker ps -a` to check all the running and exited #existed?# container. You can stop, restart and remove them. Keep in mind that you will loose all the changes that have been made to the running container after you stop it.
  ```
  docker stop give
  docker restart give
  docker rm give
