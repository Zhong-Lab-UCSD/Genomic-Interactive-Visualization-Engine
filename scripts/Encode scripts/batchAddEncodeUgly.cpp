...
int main(int argc, char *argv[]) {
	...
	try {
		...
		stmtcpb = concpb->createStatement();
...
//				cout << grpItor->first << " - " << flush;
				...
				for(map<string, map<string, string> >::const_iterator trackItor = grpItor->second.begin();
					trackItor != grpItor->second.end(); trackItor++) {
						ostringstream ostrsql, ostrsqlcp;
						subPriority++;
						...
						ostrsql << "REPLACE INTO trackDb VALUES('"
							<< trackItor->first << "', '"
							<< allValues["shortlabel"] << "', '"
							<< TRACKTYPE << "', '"
							<< allValues["longlabel"] << "', ";
						if(!hasSuperTrack && hasCompSeries) {
							// not child and is compSeries enabled
							ostrsql << 1;							// visibility	tinyint(3) unsigned
						} else {
							ostrsql << 0;							// visibility	tinyint(3) unsigned
						}
						ostrsql << ", " << ((double) PRIORITY_BASE 
							+ ((double) superPriority) 
							+ ((double) subPriority) / (double) 100)
							<< ", 0, 0, 0, 0, 0, 0, 0, 0, 0, '', '', '" 
							<< allValues["html"] << "', 'encode', 0, '"
							<< allValues["settings"] << "')";
						stmt->execute(ostrsql.str());

						// insert corresponding stuff in compbrowser
						ostrsqlcp << "REPLACE INTO TrackInfo VALUES('"
							<< trackItor->first
							<< "', '" << spcName
							<< "', '" << allValues["shortlabel"]
							<< "', '" << allValues["longlabel"]
						...
						if(hasSuperTrack) {
							...
							ostrsqlcp << "', '" << trackItor->first;
						}
						ostrsqlcp << "', '" << trackItor->second.find("url")->second
							<< "', 1)";
						stmtcpb->execute(ostrsqlcp.str());
