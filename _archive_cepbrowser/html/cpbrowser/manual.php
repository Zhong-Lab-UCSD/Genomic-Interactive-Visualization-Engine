...
<meta name="description" content="This is the manual for CEpBrowser (Comparative Epigenome Browser). In CEpBrowser, genomes from multiple species can be shown side by side to enhance the comparison of various features, such as transcript information, epigenomic modifications, SNP's, transcription factor binding sites and so on, thus aiding users in comparative genomic research." />
...
<p class="normaltext">CEpBrowser has incorporated <a href="http://www.genome.gov/10005107" target="_blank">Encyclopedia of DNA Elements</a> (ENCODE) and mouseENCODE data to help in the visualization and facilitation of deeper insights into the vast amount data within the project.</p>
...
<p class="normaltext">CEpBrowser consists of four panels: <span class="panel">gene query</span> panel, <span class="panel">gene selection</span>, <span class="panel">navigation</span>, <span class="panel">visualization</span> and two folded panels: <span class="panel">track selection</span> (See figure below) and <span class="panel">track information &amp; settings</span>. The left panels can be folded so that all the panels can be accessible under smaller screen resolution. The entire left panel group can also be hidden to further provide space for the <span class="panel">visualization</span> panel.</p>
<p class="inlineImage"><img src="images/UI.png" alt="Comparative Genome Browser Interface" width="800" height="483" /><br />
  <strong>Comparative Genomic Browser interface</strong>. The <span class="panel">gene query</span>, <span class="panel">gene selection</span>, <span class="panel">navigation</span>, <span class="panel">visualization</span> and <span class="panel">track selection</span> panels are shown.</p>
...
<p class="normaltext">CEpBrowser employs AJAX to show a gene name candidate list (see the yellow box in the figure) when a partial (at least 2 characters long) gene name is given.</p>
<p class="normaltext">The <strong>Species Selection</strong> boxes are used to select the species to visualize in the browser. Species name, the common name and database version are shown beside the boxes. At least two species need to be checked to continue and because the orthologous segment groups are anchored by human genome, human must be one of the checked. <em>Notice that because ENCODE data has only human and mouse coverage, <span class="panel">Species Selection</span> will not be available if ENCODE Datasets are selected.</em></p>
...
<p class="normaltext">After a query is submitted, the selected orthologous gene group will be shown in <span class="panel">gene selection</span> and will be instantly visualized in <span class="panel">visualization</span> as well. If partial gene name is provided as the query, all orthologous gene groups whose gene name in human partially match  the query (for example, &quot;POU5F1&quot; will match query &quot;pou&quot;, &quot;ou5f&quot;, <em>etc;</em> the match is case-insensitive) will be shown. User will need to select which orthologous gene group to visualize.</p>
...
<p class="normaltext">If there is more than one gene context for one species in an orthologous group, there will be a drop-down menu for chromosome locations of the gene contexts in that species. By selecting the chromosome locations, user can specify which gene context to be visualized. Chromosome locations with an asterisk (*) have a gene context does not overlap with any orthologous segment group, <em>i.e.</em>, no colored regions will appear in visualization. The gene name in the species will change if different gene context is selected.</p>
<p class="normaltext"><strong>Update: </strong>If you suppy genome coordinates, regions will be shown with names &quot;Region1&quot;, &quot;Region2&quot;, etc.</p>
...
<p class="normaltext">In visualization of genes, a split vision of genomes of all the selected species is shown to compare the genomes in a side-by-side view. One orthologous segment group  within a gene context will be shaded in the same color. There are a total of 16 colors in CEpBrowser (see right) so if the number of orthologous segment groups is less than 16, every group will be assigned a unique color.</p>
<p class="normaltext">Because there may be multiple gene contexts within one orthologous gene group, it is likely that some color-shaded segment does not appear in one or more species. Selecting a different chromosome location in the <span class="panel">gene selection</span> panel may help to reveal the missing segments.</p>
<p class="normaltext">The genome browser is powered by UCSC Genome Browser so virtually all functions of UCSC Genome Browser can be performed on any of the genomes in the <span class="panel">visualization</span> panel, such as zooming in/out, moving up/downstream and ordering the tracks by drag-and-drop. <strong>All tracks will be shown in a &quot;dense&quot; format, where darker means higher signal.</strong></p>
...
<p class="normaltext">In the cases when there are more than 16  orthologous segment regions within one gene context or when the orthologous segment region does not appear in some of the species, a track with all the name and the direction   of the Orthologous Segment Groups (&quot;Multi-species Alignment Track&quot;) is provided at the bottom of every species to help identify the exact orthologous relationships across species (see figure below). </p>
<p class="inlineImage"><img src="images/orthogroupname.png" alt="Orthologous Segment Group" width="611" height="407" /><br />
  <span class="panel"><strong>Visualization</strong></span><strong> Panel and Multi-species Alignment Track.</strong> Species name and information are shown on top of every sub-panel (also collapsible via clicking the name). By clicking the blue buttons to the left of every track, <span class="panel">track information &amp; settings panel</span> will pop-up. A Multi-species Alignment Track is on the bottom half of every sub-panel. This track contains information about all the orthologous segment groups. The groups with the same unique name  are orthologous  in all species and have the same color shade in all panels.</p>
...
<p class="normaltext">If desired, the view for every individual species can be collapsed by clicking the species name.</p>
...
<p class="normaltext">The <span class="panel">navigation</span> panel is provided to navigate through the selected orthologous gene group. The buttons in <strong>MASTER CONTROL</strong> are linked with all the buttons in individual species, to enable synchronized navigation. There are two types of controls in the navigation panel: sliding controls enable user to slide to the upstream/downstream region of the current view, and zooming controls allow user to zoom in/out a certain part of the genome. The direction of sliding controls in MASTER CONTROL is in regard to the gene. Therefore, if certain gene is on the Crick strand in one species (as is indicated near the gene name), going upstream in <strong>MASTER CONTROL</strong><em> will cause the view in that species moving right</em>.</p>
<table cellpadding="7" cellspacing="0" class="cautionTable">
  <tr>
    <td valign="middle" class="cautionText"><img src="images/caution.png" alt="Caution" width="40" height="36" /></td>
    <td valign="middle" class="cautionText"><ul>
        <li>As with the UCSC Genome Browser, zooming out too much will have a negative impact on response time. Please proceed with caution.</li>
      </ul></td>
  </tr>
</table>
...
<p class="inlineImage"><img src="images/trackselection.png" alt="Tracks &amp; Data Panel" width="288" height="500" /> <img src="images/trackselectionencode.png" width="447" height="500" alt="Tracks &amp; Data ENCODE version" /><br />
  <span class="panel">tracks &amp; data</span> panel. Here is the panel with all tracks shown. To download the full data file for a specific file, click the <span class="panel">download file button</span> <img src="images/download.png" width="15" height="15" alt="Download file button" />; the download link and data file type are shown in the <span class="panel">download data</span> popup. Left: three-species data version; Right: ENCODE data version</p>
<div style="clear: both;"></div>
<div class="embededImage" id="SampleSelection"><img src="images/choosesampletype.png" width="293" height="300" alt="Choose sample type panel" /><br />
  <span class="panel"><strong>Choose sample type </strong></span><strong> panel.</strong> Every sample here will control all correpsonding tracks in <span class="panel"><strong>Tracks &amp; Data</strong></span><strong></strong>.</div>
<p class="normaltext">The <span class="panel">tracks &amp; data</span> panel is used to control which of the tracks will be shown in the browser. All tracks available to all species will be shown here, and by checking the corresponding boxes, tracks can be made hidden to provide better focus. <strong>Common Tracks</strong> are tracks that exist in all species and <strong>Unique Tracks</strong> are the tracks that only exist in the selected species (many Unique Tracks are replicated experiments to provide better comparison).</p>
<p class="normaltext">For ENCODE data sets, the sample type and lab for the data will be shown in a tabular format and the <strong>Choose sample type</strong> button is used to enable users to select/deselect all tracks for the same sample (see figure to the right). </p>
...
<p class="normaltext">By clicking the blue button to the left of a track, <span class="panel">track information &amp; settings</span> panel will be shown with the UCSC Genome Browser settings page, including relevant information and a detailed control panel over the tracks. (See figure below.)</p>
<p class="inlineImage"><img src="images/trackinfo.png" alt="Orthologous Segment Group" width="622" height="391" /><br />
  <strong>Track Information &amp; Settings panel.</strong> This panel provides detailed settings for all the tracks as well as the important information on the selected track.</p>
<p class="normaltext">This panel will be automatically hidden if the changes in settings are submitted. The <strong>Close</strong> button can be used to close the panel without applying any changes in the settings.</p>
...
<p class="inlineImage"><img src="images/tablebrowser.png" alt="Orthologous Segment Group" width="638" height="437" /><br />
  <strong><span class="panel">Table Browser</span> panel.</strong> This panel, implemented by UCSC Table browser, provides controlled data downloading function together with other related functionalities.</p>
<p class="normaltext">This panel will give you the ability to download the data of regions of interest from all tracks shown in CEpBrowser. Data specific to CEpBrowser is available in &quot;Epigenomic Data from Zhong Lab and others&quot; group (as is seen in the figure). The <span class="panel">Close</span> button can be used to close the panel.</p>
